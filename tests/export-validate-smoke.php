<?php
/**
 * export_validate AJAX path (Great Vibes fragment).
 *
 * Run: php tests/export-validate-smoke.php
 *
...
$canonical = array(
	...
	'selections'        => array( 'custom_text' => 'AB 123 CD', 'font_family' => 'Great Vibes' ),
	'objects'           => array(
		array(
			'id'           => 'pckz-text',
			'role'         => 'text',
...
$debug = PCKZ_Export_Diagnostics::format_debug_suffix(
	PCKZ_Export_Diagnostics::summarize_payload( $fragment, $production_vector_svg, 'Great Vibes', 'test-url' ),
	$probe
);
if ( false === strpos( $debug, '[pckz=' ) ) {
	fwrite( STDERR, "FAIL debug suffix missing pckz version\n" );
	exit( 1 );
}

echo "OK export-validate-smoke: {$debug}\n";
exit( 0 );
